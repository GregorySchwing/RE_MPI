...
void ConfigSetup::Init(const char *fileName, ReplicaExchangeParameters *replExParams)
{
    ...
    if(line[0] == "Restart") {
      ...
      if(line.size() == 2){  
        ...
        printf("%-40s %-4.4f K\n", "Info: Input Temperature", sys.T.inKelvin);
          ...
          my_rank++;    // Add 1 to shift past Row title
          sys.T.inKelvin = stringtod(line[my_rank]);
      }
